...
def getLogger(name, file=True, path=None, visible=True,
              level=NOTSET, simple_color=False):
    try:
        getattr(sys.modules['__main__'], 'LOGGER_DICT')
    except Exception:
        setattr(sys.modules['__main__'], 'LOGGER_DICT', dict())
...
class Logger:
    ...
    def __init__(self, name='root', path=None, visible=True,
                 level=NOTSET, simple_color=False):
        self.level = level
        self.name = name
    ...
    def log(self, message, level=NOTSET):
        ...
        if True:
            if sys.modules['__main__'].LOGGER_PIPE is not None:
                ...
                f += '\033[%sm%s%s' % (LEVEL_COLORS[level],
                                       message,
                                       restore_panel_color)
                ...
                print('%s [%s]: %s' %
                      (int(time.time()), LEVEL_NAMES[level], message),
                      end="")
